...
export default function BudgetsPage() {
  ...
  const renderStatus = (status) => {
    if (status === 'Over Budget') return <span style={{ color: '#dc2626', fontSize: '0.8rem', fontWeight: 600, display: 'inline-flex', width: 'fit-content', alignItems: 'center', gap: 6, background: '#fee2e2', padding: '2px 8px', borderRadius: 4 }}><span style={{ fontSize: '10px' }}>●</span> Over Budget</span>
    if (status === 'At Risk') return <span style={{ color: '#d97706', fontSize: '0.8rem', fontWeight: 600, display: 'inline-flex', width: 'fit-content', alignItems: 'center', gap: 6, background: '#fef3c7', padding: '2px 8px', borderRadius: 4 }}><span style={{ fontSize: '10px' }}>●</span> At Risk</span>
    return <span style={{ color: '#059669', fontSize: '0.8rem', fontWeight: 600, display: 'inline-flex', width: 'fit-content', alignItems: 'center', gap: 6, background: '#d1fae5', padding: '2px 8px', borderRadius: 4 }}><span style={{ fontSize: '10px' }}>●</span> On Track</span>
  }
                                ...
                                <div style={{ position: 'relative' }}>
                                  ...
                                  {openMenuId === b.id && (
                                    <div style={{ position: 'absolute', right: 0, top: 24, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, boxShadow: '0 4px 6px rgba(0,0,0,0.1)', zIndex: 10, width: 140, overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
                                      ...
                                      <div style={{ padding: '8px 12px', fontSize: '0.85rem', cursor: 'pointer', borderBottom: '1px solid var(--border)' }} onClick={() => { setOpenMenuId(null); handleActionClick('edit', b); }}>Edit Budget</div>
